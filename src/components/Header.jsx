...
export default function Header() {
  ...
  return (
    <header
      style={solid ? { backgroundColor: 'rgba(251, 248, 242, 0.97)' } : undefined}
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ease-refined ${
        solid ? `bg-ivory-soft/95 shadow-soft ${mobileOpen ? '' : 'backdrop-blur'}` : 'bg-transparent'
      }`}
    >
      {/* Top info bar */}
      <div
      ...
      <div className="container-hotel">
        ...
        >
          <Link to="/" className="flex flex-col leading-none group">
            <span
              className={`font-display font-semibold tracking-wide transition-all duration-500 ${
                solid ? 'text-2xl text-forest-800' : 'text-3xl text-ivory [text-shadow:0_1px_10px_rgb(0_0_0_/_45%)]'
              }`}
            >
              Hotel Classic Hills
            </span>
          ...
          {/* Desktop nav */}
          <nav className={`hidden min-[980px]:flex items-center gap-8 font-sans text-sm tracking-wide ${solid ? 'text-charcoal' : 'text-ivory [text-shadow:0_1px_6px_rgb(0_0_0_/_45%)]'}`}>
      ...
      {/* Mobile menu panel — inline backgroundColor is a deliberate, redundant
          failsafe alongside the Tailwind class: this panel must always be
          fully solid over the hero image with zero chance of see-through,
          regardless of any CSS-generation or caching quirk. */}
      <div
