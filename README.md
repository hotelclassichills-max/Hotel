...
booking-policy.html      Booking Policy (some specifics marked as placeholders)
cancellation-policy.html Cancellation Policy (charges marked as placeholders)
terms-conditions.html    Terms & Conditions
privacy-policy.html      Privacy Policy
refund-policy.html       Refund Policy
hotel-rules.html          Hotel Rules & Guest Policies
...
js/main.js            Mobile nav, lightbox, gallery filters, floating weather + WhatsApp widgets, scroll-reveal animations
...
- **Weather widget wasn't displaying** — its fetch code was accidentally nested
  inside the gallery-filter-tabs check in `main.js`, so it only ran on
  `gallery.html`. It's now a top-level block that runs on every page, and the
  widget itself moved from a static homepage section to a small floating
  badge (bottom-left, all pages) that expands on hover/tap.
- **Hamburger menu didn't close** — the toggle only ever added the `open`
  class, never removed it. Rewritten to properly open/close, close on
  outside click, close on Escape, close after picking a link, lock body
  scroll while open, and animate to an X icon.
- **Blank space at the bottom on mobile** — `.hero` / `.room-hero` used plain
  `vh` units, which is the classic mobile-Safari bug (the address bar
  resizing changes `100vh` mid-scroll). Added `svh` fallbacks, `overflow-x:
  hidden` on `html`/`body`, and precise `env(safe-area-inset-bottom)`-aware
  padding for the sticky mobile bar so it doesn't leave a gap on notched
  phones.
- Added a **premium floating WhatsApp button** (bottom-right, all pages,
  gradient circle with a subtle pulse ring, expands to show "Chat with us on
  WhatsApp" on hover) — positioned to never overlap the weather widget or
  the mobile sticky bar.
- Added subtle **scroll-reveal and hover animations** (fade/slide-up on
  section headers, room and attraction cards; image zoom-on-hover; card and
  button lift) via `[data-reveal]` + `IntersectionObserver` — respects the
  site's existing `prefers-reduced-motion` rule, no animation library used.
- Added a light **photo color grade** (`brightness/contrast/saturate`)
  scoped only to real `<img>` photo elements, never to icons or logos.
- Added **6 policy pages** and a "Policies & Information" link row in every
  page's footer. Specific figures (cancellation charges, refund percentages,
  child/extra-bed rates, smoking/pet rules, etc.) are marked inline as
  placeholders — nothing hotel-specific was invented.
- Added the **reservations@hotelclassichills.in** email, clickable, to the
  contact page, FAQ schema, and every page's footer.
